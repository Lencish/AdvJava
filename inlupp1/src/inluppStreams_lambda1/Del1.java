...
public class Del1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("1. " + Arrays.stream(austin).filter(s -> s.length() > 8).count() 
				+ " ord fler än 8 bokstäver.");

		System.out.println("2. " + Arrays.stream(austin).distinct().count() 
				+ " unika ord.");

		System.out.println("3. " + Arrays.stream(austin).filter(s -> s.length() < 4).count() 
						+ " ord färre än 4 tecken.");

		System.out.println("4. " + Arrays.stream(austin).filter(s -> s.length() > 8).distinct().count()
				+ " unika ord fler än 8 bokstäver.");

		System.out.println("5. " + Arrays.stream(austin).mapToDouble(s -> s.length()).average().getAsDouble()
				+ " är den genomsnittliga ordlängden.");

		System.out.println("6. " + Arrays.stream(austin).mapToInt(s -> s.length()).sum() 
				+ " tecken finns i listan.");

		System.out.println("7. a) Är alla ord kortare än 12 tecken? ("
				+ (Arrays.stream(austin).filter(s -> s.length() > 12).count() > 0 ? "Ja" : "Nej")
				+ ")\n   b) Är all ord längre än två bokstäver? ("
				+ (Arrays.stream(austin).filter(s -> s.length() < 2).count() > 0 ? "Ja" : "Nej") + ")");
	}
